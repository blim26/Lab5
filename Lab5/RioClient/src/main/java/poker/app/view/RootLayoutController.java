...
public class RootLayoutController implements Initializable {
	...
	public void BuildMenus()
	{
		...
		Menu mnuBet = new Menu();
		mnuBet.setText("Pick Betting");
		mb.getMenus().add(1,mnuBet);
		
		ToggleGroup tglGrpBet = new ToggleGroup();
		
		for (eBet eBet : eBet.values()) {
			RadioMenuIten rmi = new RadioMenuItem(eBet.toString());
			rmi.setId("PokerBetting" + String.valueOf(eBet.getBet()));
	...
	public String getBetRuleName() {
		String strBetRuleID = null;
		
		for (Menu m: mb.getMenus()) 
		{
			if (m.getText*() == "Pick Betting") 
			{
				for (MenuItem mi:m.getItems()) 
				{
					if (mi.getClass().equlas(RadioMenuItem.class))
					{
						RadioMenuItem rmi = (RadioMenuItem)mi;
						if (rmi.isSelected() == true)
						{
							strBetRuleID = rmi.getId();
							break;
						}
